...
class GCRM:
    def __init__(self, n_species, n_resources, spec, spec_var, n_families, results_path, dcase=None,
        ...
        match dcase:
            case "random_refined":
                ...
                n_levels = 1 + int(np.sqrt(n_resources))
                ...
                self.W = np.zeros(n_resources)
                for b in range(1, n_resources):
                    if levels[b] == levels[b-1]:
                        self.W[b] = self.W[b-1]
                    else:
                        self.W[b] = self.W[b-1] * 2 + abs(rng.normal(loc=1, scale=0.5, size=1)[0])
                ...
                for i in range(n_resources):
                    for j in range(n_resources):
                        if levels[i] - levels[j] > 0:
                            ...
                            self.W_ba[i, j] = self.W[i] - self.D[i, j] * self.W[j]
    ...
    def time_development(self, t, y):
        ...
        self.realised_syntrophy.append(all_consumption[1:-1].sum() / all_production[:-1].sum())
        ...
        species_next = all_grm

        resources_next = (self.alpha - resources) / self.tau - all_consumption + all_production
    ...
    def grow_ivp(self, run_id, update=True, plot=False, savefigs=True, repeat=False, set_y0=np.array(False),
                 t_end=1000, resolution=0.1, report=True, output=False):
            ...
            while t_span[0] < t_end:
                while t_span[0] < self.next_invasion_time:
                    ...
                    if all_t.size == 0:  # If result arrays are empty still, populate them with the first results
                        ...
                        all_MCI = np.concatenate((all_MCI, self.MCI), axis=0)
                        all_t_out = np.concatenate((all_t_out, self.t), axis=0)
                    ...
                    if sol.t_events[0].size > 0:
                        ...
                        print(f"Reached next invasion time:{sol.t[-1]}")
                ...
                if self.number_of_invasions <= self.n_invaders:
                    ...
                    print(f"Number of invasions: {self.number_of_invasions}")
                else:
        ...
        if output:
            common_timesteps = np.linspace(0, t_end, 1000)
            print("len t:", len(all_t))
            print("len MCI", len(all_MCI))
            interp_MCI = np.interp(common_timesteps, all_t_out, all_MCI)
            generalists = np.zeros(len(common_timesteps))
            specialists = np.zeros(len(common_timesteps))
            interp_species = []
            for i in range(self.n_players):
                print("len t:", len(all_t))
                print("len y:", len(all_y[i]))
                interp = np.interp(common_timesteps, all_t, all_y[i])
                interp_species.append(interp)
                if self.n_reactions_fam[int(self.family_ids[int(self.players[i])])] == int(self.n_resources/5):
                    generalists += interp
                else:
                    specialists += interp
            output_object = Object()
            output_object.gen = generalists
            output_object.spec = specialists
            output_object.MCI = interp_MCI
    ...
    def create_report(self, sol, run_id):
        ...
        line2 = f"<p>No unidirectional flow</p>"
    ...
    def plot_generation(self, sol, savefigs, run_id):
        ...
        gen_spec = ["gen" if self.n_reactions_fam[i] == int(self.n_resources/2) else "spec" for i in range(self.n_families)]
...
foo = GCRM(n_species=1, n_resources=10, spec=0.5, spec_var=0.05, n_families=10,
           results_path=folder_name, dcase="nullmodel", mcase=None, cutoffvalue=0.0001,
           m_val=0.1, invade=True, t_invaders=25, first_invasion=25, n_invaders=30, h=2)

invaded = []
survived = []
t_end = 1000
timepoints = np.linspace(0, t_end, 1000)
generalists = np.zeros(len(timepoints))
specialists = np.zeros(len(timepoints))
MCI = np.zeros(len(timepoints))

current_time = datetime.now()
run_id = current_time.strftime("%Y%m%d_%H%M%S")
output = foo.grow_ivp(run_id=run_id, plot=False, savefigs=True, t_end=t_end, resolution=0.5, output=True)
generalists += output.gen
specialists += output.spec
MCI += output.MCI
invaded.append(foo.get_invader_families())
...
for i in range(20):
    ...
    current_time = datetime.now()
    run_id = current_time.strftime("%Y%m%d_%H%M%S")
    output = foo.grow_ivp(run_id=run_id, plot=False, savefigs=True, t_end=t_end, resolution=0.5, repeat=True, output=True)
    generalists += output.gen
    specialists += output.spec
    MCI += output.MCI
    invaded.append(foo.get_invader_families())
...
fig1, loc1 = plt.subplots(nrows=1, ncols=1, figsize=(16, 9))
loc1.plot(timepoints, MCI)
loc1.set_title("MCI")
fig1.savefig(f'{folder_name}/fig1')
plt.close()

fig2, loc2 = plt.subplots(nrows=1, ncols=1, figsize=(16, 9))
loc2.plot(timepoints, generalists)
loc2.plot(timepoints, specialists)
loc2.set_title("Generalist-Specialist ratio")
fig2.savefig(f'{folder_name}/fig2')
plt.close()

fig3, loc3 = plt.subplots(nrows=1, ncols=2, figsize=(16, 9))

invaded_flattened = [item for sublist in invaded for item in sublist]
invaded_counts = Counter(invaded_flattened)
labels, values = zip(*invaded_counts.items())
loc3[0].bar(labels, values)

survived_flattened = [item for sublist in survived for item in sublist]
survived_counts = Counter(survived_flattened)
labels, values = zip(*survived_counts.items())
loc3[1].bar(labels, values)

fig3.suptitle("Number of times a family has successfully invaded (Left) or survived (Right) among all runs")
fig3.savefig(f'{folder_name}/fig3')
plt.close()
